...
class SampleDataInput(BaseModel):
    index_name: str = Field(..., description="Name of the Elasticsearch index")
    size: int = Field(5, description="Number of sample documents to retrieve")
...
def get_sample_data(index_name: str, size: int = 5) -> str:
...
def elastic_search(index_name: str, query: str, size: int = 10) -> str:
...
elastic_tools = [
    ...
    Tool.from_function(
        ...
        description="Get sample documents from an index to understand its structure",
        args_schema=SampleDataInput
    ),
    Tool.from_function(
        func=elastic_search,
        name="elastic_search",
        description="Execute search queries against Elasticsearch",
...
def create_agent(llm, tools, system_message: str):
    prompt = ChatPromptTemplate.from_messages([
        ...
        3. Prefix final answers with "FINAL ANSWER:"
        4. Available tools: {", ".join([t.name for t in tools])}
        {system_message}"""),
        MessagesPlaceholder(variable_name="messages")
    ])
    return prompt | llm.bind_tools(tools)
...
elastic_agent = create_agent(
    cfg.llm, 
    elastic_tools,
    "You are an Elasticsearch expert. Use the tools to explore and query Elasticsearch data."
)

sql_agent = create_agent(
    cfg.llm,
    sql_tools,
    "You are a SQL expert. Use the tools to query the SQL database."
)
...
def router(state):
    ...
    if "FINAL ANSWER" in last_message.content:
        return "end"
    if len(messages) > cfg.max_recursion:
        return "end"
    if "function_call" in last_message.additional_kwargs:
        return "call_tool"
    return "continue"
...
def execute_query(question: str):
    try:
        ...
        return {"error": "No final answer found within recursion limit"}
    except Exception as e:
...
if __name__ == "__main__":
    load_dotenv()
    question = (
        "Find customers in the 'customers' index with purchases over $1000 "
        "from the SQL database, and return their contact information"
    )
