...
elastic_tools = [
    StructuredTool.from_function(list_indices, name="elastic_list_indices", description="Lists all available Elasticsearch indices", args_schema=ListIndicesInput),
    StructuredTool.from_function(get_index_details, name="elastic_index_details", description="Gets details about an Elasticsearch index", args_schema=IndexDetailsInput),
    StructuredTool.from_function(get_index_data, name="elastic_index_data", description="Gets sample documents from an index", args_schema=IndexDataInput),
    StructuredTool.from_function(elastic_search, name="elastic_search", description="Runs queries on Elasticsearch index", args_schema=SearchToolInput)
]
...
elastic_prompt = ChatPromptTemplate.from_messages([
    SystemMessage(content="You are an Elasticsearch assistant."),
    ("user", "{input}"),
    MessagesPlaceholder(variable_name="agent_scratchpad")
])

elastic_agent = create_openai_functions_agent(cfg.llm, elastic_tools, prompt=elastic_prompt)
elastic_executor = AgentExecutor(agent=elastic_agent, tools=elastic_tools, verbose=True, handle_parsing_errors=True, return_intermediate_steps=True)
...
def router_node(state: AgentState) -> str:
    ...
    if any(k in q for k in ["index", "elasticsearch", "document"]):
        return "elastic_agent"
    return "sql_agent"

def sql_agent_node(state: AgentState) -> AgentState:
    result = sql_agent.invoke({"input": state["question"]})
    return {"question": state["question"], "response": result["output"]}

def elastic_agent_node(state: AgentState) -> AgentState:
    result = elastic_executor.invoke({"input": state["question"]})
    return {"question": state["question"], "response": result["output"]}
...
graph.add_node("elastic_agent", elastic_agent_node)
graph.set_entry_point("router")
graph.add_conditional_edges("router", lambda state: router_node(state), {
    "sql_agent": "sql_agent",
    "elastic_agent": "elastic_agent"
})
graph.add_edge("sql_agent", END)
graph.add_edge("elastic_agent", END)
workflow = graph.compile()

# === Run Example ===
response = workflow.invoke({"question": "Show me all records from material table"})
